  ...
  return (
    <div id="backup" ref={sectionRef} className="gradient-card rounded-xl border border-border p-5 space-y-4 animate-fade-in [animation-delay:120ms] scroll-mt-20">
      ...
      <p className="text-xs text-muted-foreground">
        Exportiere alle deine Daten als JSON-Backup oder CSV-Dateien f&uuml;r Excel.
      </p>
      {lastExportDate ? (
          ...
          <div className="text-xs">
            ...
            {daysSinceLastExport !== null && daysSinceLastExport > 90 && (
              <span className="text-destructive font-medium ml-1">
                &mdash; &#9888; {daysSinceLastExport} Tage her! Bitte erstelle ein neues Backup.
              </span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 p-2.5 rounded-lg border border-gold/30 bg-gold/5">
          ...
          <span className="text-xs text-gold font-medium">Noch kein Export durchgef&uuml;hrt &mdash; erstelle jetzt ein Backup!</span>
        </div>
      )}
      <DataExportBackup />
    </div>
