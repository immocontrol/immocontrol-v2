...
export function MietvertragCreateAndSign() {
  ...
  const buildPdfBlob = useCallback(async (): Promise<{ blob: Blob; fileName: string } | null> => {
    try {
      ...
      doc.text(sanitizeForPdf("Mietvertrag"), margin, y); y += lineH + 6;
      doc.setFontSize(10);
      doc.setFont("helvetica", "normal");
      doc.text(sanitizeForPdf(`Zwischen ${landlordName || "[Vermieter]"} ("Vermieter") und ${contractData.tenantName} ("Mieter").`), margin, y); y += lineH + 4;
      doc.setFont("helvetica", "bold");
      doc.text(sanitizeForPdf("§ 1 Mietobjekt"), margin, y); y += lineH;
      doc.setFont("helvetica", "normal");
      doc.text(sanitizeForPdf(`Vermietet wird: ${address || "[Adresse]"}.`), margin, y); y += lineH + 4;
      doc.setFont("helvetica", "bold");
      doc.text(sanitizeForPdf("§ 2 Mietdauer"), margin, y); y += lineH;
      doc.setFont("helvetica", "normal");
      doc.text(sanitizeForPdf(`Beginn: ${startFormatted}. Das Mietverhältnis wird auf unbestimmte Zeit geschlossen.`), margin, y); y += lineH + 4;
      doc.setFont("helvetica", "bold");
      doc.text(sanitizeForPdf("§ 3 Miete"), margin, y); y += lineH;
      doc.setFont("helvetica", "normal");
      doc.text(sanitizeForPdf(`Kaltmiete: ${formatCurrency(contractData.coldRent)}/Monat. Nebenkosten-Vorauszahlung: ${formatCurrency(contractData.nkPrepayment)}/Monat.`), margin, y); y += lineH + 4;
      doc.setFont("helvetica", "bold");
      doc.text(sanitizeForPdf("§ 4 Kaution"), margin, y); y += lineH;
      doc.setFont("helvetica", "normal");
      doc.text(sanitizeForPdf(`Kaution: ${formatCurrency(contractData.deposit)}.`), margin, y); y += lineH + 4;
      doc.setFont("helvetica", "bold");
      doc.text(sanitizeForPdf("§ 5 Kündigung"), margin, y); y += lineH;
      doc.setFont("helvetica", "normal");
      doc.text(sanitizeForPdf(`Kündigungsfrist: ${noticeMonths} Monate zum Monatsende (§ 573c BGB).`), margin, y); y += lineH + 12;
      doc.setDrawColor(180);
      doc.line(margin, y, margin + 75, y); doc.line(pageW - margin - 75, y, pageW - margin, y); y += lineH;
      doc.setFontSize(9);
      doc.setTextColor(120);
      doc.text(sanitizeForPdf("Vermieter"), margin, y); doc.text(sanitizeForPdf("Mieter"), pageW - margin - 75, y);
      doc.setFontSize(8);
      doc.text(sanitizeForPdf(`Erstellt mit ImmoControl · ${today}`), margin, 285);
      doc.setTextColor(0);
