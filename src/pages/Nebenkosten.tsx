...
<div class="footer">ImmoControl · Nebenkostenabrechnung · Erstellt am ${new Date().toLocaleDateString("de-DE")}</div>
  ...
  const generateBillingPdfBlob = useCallback(async (
    ...
    doc.text(sanitizeForPdf("Nebenkostenabrechnung"), m, y); y += 10;
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(sanitizeForPdf(`Objekt: ${property?.name || "-"} ${property?.address || ""}`), m, y); y += 6;
    if (tenant) { doc.text(sanitizeForPdf(`Mieter: ${tenant.first_name} ${tenant.last_name}`), m, y); y += 6; }
    doc.text(sanitizeForPdf(`Zeitraum: ${new Date(billing.billing_period_start).toLocaleDateString("de-DE")} - ${new Date(billing.billing_period_end).toLocaleDateString("de-DE")}`), m, y); y += 12;
    doc.setFont("helvetica", "bold");
    doc.text(sanitizeForPdf("Kostenart"), m, y);
    doc.text(sanitizeForPdf("Beschreibung"), m + 40, y);
    doc.text(sanitizeForPdf("Mieteranteil"), m + 120, y); y += 6;
    doc.setFont("helvetica", "normal");
    items.forEach(i => {
      doc.text(sanitizeForPdf(i.category), m, y);
      doc.text(sanitizeForPdf((i.description || "").slice(0, 25)), m + 40, y);
      doc.text(sanitizeForPdf(formatCurrency(Number(i.tenant_amount))), m + 120, y); y += 6;
    });
    y += 6;
    doc.setFont("helvetica", "bold");
    doc.text(sanitizeForPdf("Summe Mieteranteil"), m, y);
    doc.text(sanitizeForPdf(formatCurrency(Number(billing.tenant_share))), m + 120, y); y += 6;
    doc.setFont("helvetica", "normal");
    doc.text(sanitizeForPdf("Vorauszahlungen"), m, y);
    doc.text(sanitizeForPdf(`-${formatCurrency(Number(billing.prepayments))}`), m + 120, y); y += 8;
    doc.setFont("helvetica", "bold");
    doc.text(sanitizeForPdf(Number(billing.balance) >= 0 ? "Nachzahlung" : "Guthaben"), m, y);
    doc.text(sanitizeForPdf(formatCurrency(Math.abs(Number(billing.balance)))), m + 120, y);
    y += 14;
    doc.setFontSize(7);
    doc.setFont("helvetica", "normal");
    doc.text(sanitizeForPdf("Die Abrechnung erfolgt nach der Betriebskostenverordnung (BetrKV). Gemäß § 259 Abs. 3 BGB ist die"), m, y);
    doc.text(sanitizeForPdf("Abrechnung dem Mieter bis zum Ablauf des 12. Monats nach Ende des Abrechnungszeitraums mitzuteilen."), m, y + 4);
    doc.text(sanitizeForPdf("Einwendungen können innerhalb von 12 Monaten nach Zugang geltend gemacht werden."), m, y + 8);
    doc.setFontSize(8);
    doc.text(sanitizeForPdf(`ImmoControl · Erstellt am ${new Date().toLocaleDateString("de-DE")}`), m, 285);
