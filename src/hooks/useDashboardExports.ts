/**
 * useDashboardExports — extracted from Dashboard.tsx (Fix 3: Split large files).
 * CSV and PDF export logic for the portfolio dashboard.
 */
...
export function useDashboardExports(properties: PropertyForExport[], stats: PortfolioStats) {
  const exportCSV = useCallback(() => {
    if (properties.length === 0) return;
    const headers = [
      "Name", "Adresse", "Typ", "Einheiten", "Kaufpreis", "Aktueller Wert",
      "Miete/M", "Kosten/M", "Kreditrate/M", "Cashflow/M", "Cashflow/J",
      "Brutto-Rendite %", "Netto-Rendite %", "Restschuld", "Zinssatz", "m\u00B2", "Baujahr", "Besitz",
    ];
